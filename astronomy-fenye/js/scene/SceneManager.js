import { raDecToVector3, eclipticToEquatorial, equatorialToHorizontal } from '../data/starData.js';
...
export class SceneManager {
  ...
  init() {
    ...
    this.celestialSphereGroup.add(this.constellationGroup);
    this.constellationGroup.visible = false;

    this.fourSymbolsGroup = new THREE.Group();
    this.celestialSphereGroup.add(this.fourSymbolsGroup);
  ...
  createEquatorialMeridians() {
    for (let ra = 0; ra < 360; ra += 30) {
      ...
      for (let dec = -90; dec <= 90; dec += 5) {
        const pos = raDecToVector3(ra, dec, this.sphereRadius);
        points.push(pos);
      }
      const geometry = new THREE.BufferGeometry().setFromPoints(points);
  ...
  createEclipticGrid() {
    ...
    for (let i = 0; i <= 360; i += 2) {
      const eq = eclipticToEquatorial(i, 0);
      const pos = raDecToVector3(eq.ra, eq.dec, this.sphereRadius);
  ...
  addPoleMarker(label, dec, color, isEcliptic = false) {
    const pos = raDecToVector3(0, dec, this.sphereRadius + 2);
  ...
  addCircleLabel(text, ra, dec, color) {
    const pos = raDecToVector3(ra, dec, this.sphereRadius + 3);
  ...
  addDegreeLabel(text, ra, dec, color) {
    const pos = raDecToVector3(ra, dec, this.sphereRadius + 2);
  ...
  addZodiacLabel(text, ra, dec, color) {
    const pos = raDecToVector3(ra, dec, this.sphereRadius + 5);
  ...
  createFourSymbols() {
    const fourSymbols = [
      ...
      },
      {
        name: '北方玄武',
        color: 0x4488ff,
        mansions: ['dou', 'niu', 'nü', 'xu', 'wei_north', 'shi', 'bi_north'],
        labelRA: 325,
        labelDec: 5
      },
      {
        name: '西方白虎',
        color: 0xffffff,
        mansions: ['kui', 'lou', 'wei_west', 'mao', 'bi_west', 'zui', 'shen'],
        labelRA: 45,
        labelDec: 18
      },
      {
    ...
    fourSymbols.forEach(symbol => {
      const group = new THREE.Group();
      group.userData = { symbol: symbol.name };

      const boundaryPoints = [];
      symbol.mansions.forEach(mansionId => {
        const star = this.starGroup.children.find(
          c => c.userData && c.userData.mansion && c.userData.mansion.id === mansionId
        );
        if (star) {
          boundaryPoints.push(star.position.clone().normalize().multiplyScalar(this.sphereRadius + 1));
        }
      });

      if (boundaryPoints.length >= 3) {
        const curve = new THREE.CatmullRomCurve3(boundaryPoints, true);
        ...
        line.userData = { noClipping: true };
        group.add(line);

        const fillGeo = new THREE.BufferGeometry().setFromPoints(curvePoints);
        ...
        fillMesh.userData = { noClipping: true };
        group.add(fillMesh);
      }

      const labelPos = raDecToVector3(symbol.labelRA, symbol.labelDec, this.sphereRadius + 10);
      const canvas = document.createElement('canvas');
      const ctx = canvas.getContext('2d');
      canvas.width = 256;
      canvas.height = 80;
      ctx.font = 'bold 26px SimHei, Microsoft YaHei, sans-serif';
      ctx.fillStyle = '#' + symbol.color.toString(16).padStart(6, '0');
      ctx.shadowColor = '#' + symbol.color.toString(16).padStart(6, '0');
      ctx.shadowBlur = 12;
      ...
      this.fourSymbolsGroup.add(group);
    });

    this.fourSymbolsGroup.visible = false;
  }
  ...
  createBackgroundStars() {
    ...
    for (let i = 0; i < 2000; i++) {
      ...
      const pos = raDecToVector3(ra, dec, this.sphereRadius * 0.99);
  ...
  addStarLabel(text, ra, dec, color = 0xffffff) {
    ...
    sprite.position.copy(pos);
    sprite.scale.set(12, 3, 1);
    sprite.userData = { ra, dec, isLabel: true };
  ...
  highlightStar(starId) {
    ...
    this.highlightConstellation(starId);
  }
  ...
  resetHighlights() {
    ...
    this.resetConstellationHighlights();
  }
  ...
  createConstellationLines(twentyEightMansions, constellationLineTopology) {
    twentyEightMansions.forEach(mansion => {
      const topology = constellationLineTopology[mansion.id];
      if (!topology || !mansion.stars || mansion.stars.length < 2) return;

      topology.forEach(lineIndices => {
        if (lineIndices.length !== 2) return;
        const star1 = mansion.stars[lineIndices[0]];
        const star2 = mansion.stars[lineIndices[1]];
        if (!star1 || !star2) return;

        const pos1 = raDecToVector3(star1.ra, star1.dec, this.sphereRadius);
        const pos2 = raDecToVector3(star2.ra, star2.dec, this.sphereRadius);
  ...
  highlightConstellation(mansionId) {
    this.constellationGroup.children.forEach(child => {
      if (child.userData && child.userData.mansionId === mansionId) {
        child.material.color.setHex(0xffff00);
  ...
  toggleLayer(layerName, visible) {
    switch (layerName) {
      ...
      case 'constellation':
        ...
        this.fourSymbolsGroup.visible = visible;
        break;
      case 'grid':
